  ...
  <section class="form flex-4 wrap-reverse">
    ...
    <div class="container center">
      ...
      <form id="incluir" action="ManterPratoController?acao=confirmarOperacao&operacao=${operacao}"
            ...
            <div class="col-sm-2">
              <select name="txtIdEstabelecimento" class="custom-select mr-sm-2">
                ...
                <c:forEach items="${estabelecimentos}" var="estabelecimento">
                  <option value="${estabelecimento.id}" <c:if test="${prato.idEstabelecimento == estabelecimento.id}">
                    selected</c:if>
        ...
        <div class="row">
          <label for="dataCriacao" class="col-sm-2 col-form-label">Data da Criação
            <div class="col-sm-2">
              <input type="date" class="form-control" name="txtDataCriacao" id="dataCriacao"
                value="${prato.dataCriacao}" <c:if test="${operacao == 'Excluir'}"> readonly</c:if>/>
            </div>
          </label>
        </div>
        ...
        <div>
          ...
          <a href="/PesquisarPratoController"><i class="fad fa-chevron-left"></i> Voltar</a>
        </div>
      </form>
    </div>
    </div>
